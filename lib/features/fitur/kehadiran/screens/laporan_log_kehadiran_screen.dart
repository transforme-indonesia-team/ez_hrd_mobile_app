...
class LaporanLogKehadiranScreen extends StatefulWidget {
  const LaporanLogKehadiranScreen({super.key});

  @override
  State<LaporanLogKehadiranScreen> createState() =>
      _LaporanLogKehadiranScreenState();
}

class _LaporanLogKehadiranScreenState extends State<LaporanLogKehadiranScreen> {
  DateTimeRange? _selectedDateRange;
  List<MemberData> _selectedEmployees = [];

  String? _selectedKodeMesin;
  String? _selectedStatusDaring;

  final List<String> _kodeMesinOptions = [
    'Pilih',
    'Mesin Utama',
    'Mesin Cabang A',
    'Mesin Cabang B',
  ...
  final List<String> _statusDaringOptions = [
    'Pilih',
    'Online',
    'Offline',
  ];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildLabel(colors, 'Kode Mesin'),
            SizedBox(height: 4.h),
            GestureDetector(
              onTap: () {
                _showDropdownBottomSheet(
                  title: 'Pilih Kode Mesin',
                  options: _kodeMesinOptions,
                  onSelected: (val) => setState(() => _selectedKodeMesin = val),
                );
              },
              child: _buildTextField(
                colors,
                hint: _selectedKodeMesin == null || _selectedKodeMesin == 'Pilih'
                    ? 'Pilih'
                    : _selectedKodeMesin!,
            ...
            _buildLabel(colors, 'Status Daring'),
            SizedBox(height: 4.h),
            GestureDetector(
              onTap: () {
                _showDropdownBottomSheet(
                  title: 'Pilih Status Daring',
                  options: _statusDaringOptions,
                  onSelected: (val) =>
                      setState(() => _selectedStatusDaring = val),
                );
              },
              child: _buildTextField(
                colors,
                hint: _selectedStatusDaring == null ||
                        _selectedStatusDaring == 'Pilih'
                    ? 'Pilih'
                    : _selectedStatusDaring!,
                suffixIcon: Icons.keyboard_arrow_down,
              ),
            ),
            SizedBox(height: 12.h),
